...
var initCmd = &cobra.Command{
	Use:   "init",
	Short: "Create starter config at ~/.invossh/servers.json (no real secrets)",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := out.Add(config.Server{
			Name:        "example-host",
			Description: "示例：请 delete 后用 invossh add 写入真实主机（勿保留 REPLACE_ME）",
			Host:        "192.0.2.10",
			Port:        22,
			User:        "root",
			Password:    "REPLACE_ME",
		...
		fmt.Printf("wrote %s\n", path)
		fmt.Println("next: invossh add --host <ip> --user root --password <secret> --desc \"...\"")
